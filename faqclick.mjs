...
async function main() {
  ...
  await send('Runtime.evaluate', { expression: `document.querySelector('.faq-wrap').scrollIntoView({block:'center'})` });
  await new Promise(r => setTimeout(r, 300));
  // click the 2nd faq question
  await send('Runtime.evaluate', { expression: `document.querySelectorAll('.faq-item__q')[1].click()` });
  await new Promise(r => setTimeout(r, 400));
  const check = await send('Runtime.evaluate', { expression: `document.querySelectorAll('.faq-item')[1].classList.contains('is-open') + ' | aria=' + document.querySelectorAll('.faq-item__q')[1].getAttribute('aria-expanded')` });
  console.log('after click state:', check.result.result.value);
